...
import org.company.msaccount.feign.CardClient;
...
import org.company.msaccount.model.CardRequest;
...
public class UserService {
    ...
    public UserService(UserRepository userRepository, RestTemplate restTemplate, CardClient cardClient) {
    ...
    public Long saveCard(CardRequest cardRequest) {
       return restTemplate.postForObject("http://localhost:8080/v1/cards", cardRequest, Long.class);
    ...
    public void updateUserPartially(Long userId, String mail, String password) {
        ...
        Users users = userRepository.findByIdAndActive(userId, ActiveStatus.ACTIVE).map(userss -> {
    ...
    public CardResponse getCardById(Long cardId) {
        log.info("Actionlog.getCardById .start with userId={}", cardId);
        String url = "http://localhost:8080/v1/cards/" + cardId;
        CardResponse cardResponse = restTemplate.getForObject(url, CardResponse.class);
        return cardResponse;
    }
    public CardResponse findCardById(Long cardId) {
        log.info("Actionlog.findCardById .start with cardId={}", cardId);
        CardResponse cardResponse = cardClient.getCard(cardId);
        log.info("Actionlog.findCardById .end with cardId={}", cardId);
        return cardResponse;
    }
}
